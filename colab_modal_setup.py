...
env = " ".join(f"{k}={userdata.get(k)}" for k in ("SUPABASE_URL", "SUPABASE_SERVICE_KEY", "R2_ACCOUNT_ID", "R2_ACCESS_KEY", "R2_SECRET_KEY", "R2_BUCKET", "R2_PUBLIC_URL"))
...
for d in ("models", "reference", "cache"):
    if os.path.isdir(f"{ROOT}/{d}"):
        r = subprocess.run(f"modal volume put ipanema-data {ROOT}/{d} /match_analysis/{d} --force", shell=True, capture_output=True, text=True); print(d, "->", (r.stdout or r.stderr)[-120:])
print("modal ready")
