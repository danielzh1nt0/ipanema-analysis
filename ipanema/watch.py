...
def step(state, ROOT, CODE, REPO):
    ...
    if last_run and code_sha[:7] in last_run and state.get("first_done"): return          # newest code commit already has results
    ...
    # retrain the ball detector when new ball labels exist, then re-detect the ball on every clip
    try:
        import glob as _g
        ...
        recipe = "r2"; last_recipe = _j.load(open(manifest)).get("recipe") if os.path.exists(manifest) else None
        if labels and (not os.path.exists(weights) or n_labels != last_n or recipe != last_recipe):
