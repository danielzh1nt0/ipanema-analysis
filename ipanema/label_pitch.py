...
<script>
const IDX={json.dumps(list(imgs))},IM={json.dumps(imgs)},V={json.dumps(verts)},E={json.dumps(edges)},L={L},Wd={W},SX={W_full/1280},SY={H_full/int(1280*H_full/W_full)};
...
function drawMini(){{mx.clearRect(0,0,m.width,m.height);mx.strokeStyle='#cfc';mx.lineWidth=1.5;for(const e of E){{const a=px(V[e[0]]),b=px(V[e[1]]);mx.beginPath();mx.moveTo(a[0],a[1]);mx.lineTo(b[0],b[1]);mx.stroke();}}
 const done=gt[IDX[fi]]||{{}};V.forEach((v,i)=>{{const p=px(v);mx.beginPath();mx.arc(p[0],p[1],i===ki?8:4,0,7);mx.fillStyle=i===ki?'#ffd54f':(done[i]===null?'#888':(done[i]?'#5cf':'#fff'));mx.fill();if(i===ki){{mx.fillStyle='#000';mx.font='bold 11px monospace';mx.fillText(i,p[0]-4,p[1]+4);}}}});}}
...
function status(){{const done=gt[IDX[fi]]||{{}};const nl=Object.values(done).filter(v=>v).length;document.getElementById('p').textContent=`Frame ${{fi+1}}/${{IDX.length}} (video frame ${{IDX[fi]}}) · landmark ${{ki}}/${{V.length-1}} · ${{nl}} clicked`;
