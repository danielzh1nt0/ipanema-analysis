...
def ensure_finetuned(root, videos_dir, log=print):
    ...
    last = json.load(open(manifest_path(root))).get("n") if os.path.exists(manifest_path(root)) else -1
    if labels and (not os.path.exists(weights_path(root)) or n != last):
        if train(root, videos_dir, log=log): json.dump({"n": n}, open(manifest_path(root), "w")); return True
    return False
