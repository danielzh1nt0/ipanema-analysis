...
def prepare(video_src, match_id, S, log=print, train_ball=True, debug=True):
    ...
    else: per, fps = TR.track(video, S.weights["player"], H, tm, S.conf_player, log=log, imgsz=1920 if panorama else None)   # panorama: whole frame at full resolution (measured 12:29: 23 players/frame at 7.4 frames/s; 6 tiles found 24 at 1.8 frames/s); pickle.dump((per, fps), open(trk, "wb"))
