...
try:
    ...
    s = res["summary"]; json.dump(s, open(f"{OUT}/summary.json", "w"), indent=1); n, o = s["new"], s["old"]
    g = (f"ball exam (40 frames Daniel clicked, never trained on): NEW {n['correct']}/{n['of']} correct "
         f"(ball found {n['ball_found']}/{n['ball_frames']}, no-ball right {n['no_ball_right']}/{n['no_ball_frames']}); OLD {o['correct']}/{o['of']} (ball found {o['ball_found']}/{o['ball_frames']})")
