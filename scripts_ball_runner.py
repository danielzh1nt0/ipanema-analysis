...
try:
    ...
    g = (f"ball exam (40 frames Daniel clicked, never trained on): NEW {n['correct']}/{n['of']} correct "
         ...
         + (f"NEW + far zoom {fz['correct']}/{fz['of']} (ball found {fz['ball_found']}/{fz['ball_frames']}); " if fz else "") + f"OLD {o['correct']}/{o['of']} (ball found {o['ball_found']}/{o['ball_frames']})" + (f"; ROUND-1 detector {s['round1']['correct']}/{s['round1']['of']}" if s.get("round1") else ""))
